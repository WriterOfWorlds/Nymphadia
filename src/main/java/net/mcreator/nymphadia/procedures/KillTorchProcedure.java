...
public class KillTorchProcedure {
	@SubscribeEvent
		...
		if ((world.getBlockState(new BlockPos((int) x, (int) y, (int) z))).getBlock() == Blocks.TORCH
				|| (world.getBlockState(new BlockPos((int) x, (int) y, (int) z))).getBlock() == Blocks.WALL_TORCH) {
			{
				...
				for (Entity entityiterator : _entfound) {
					if (entityiterator.getPersistentData().getBoolean("isTorch")) {
						if (!entityiterator.level.isClientSide())
							entityiterator.discard();
						if (!world.isClientSide()) {
							MinecraftServer _mcserv = ServerLifecycleHooks.getCurrentServer();
							if (_mcserv != null)
								_mcserv.getPlayerList().broadcastMessage(new TextComponent("Torch was broken."), ChatType.SYSTEM, Util.NIL_UUID);
						}
					}
				}
			}
